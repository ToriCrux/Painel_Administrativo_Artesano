...
import { useGerenciarCategorias } from "../hooks/useGerenciarCategorias";

export default function ExcluirCategoriasPage() {
  const {
    categorias,
    loading,
    error,
  ...
  return (
    <Wrapper>
      <HeaderBar>
        <Breadcrumb>FILTROS / CATEGORIA / GERENCIAR</Breadcrumb>
      </HeaderBar>
        ...
        <table className="w-full border-collapse bg-white shadow-md rounded-lg overflow-hidden text-sm sm:text-base">
          ...
          <tbody>
            {categorias.length === 0 ? (
              ...
              categorias.map((cat) => (
                <tr key={cat.id} className="border-b hover:bg-gray-50 transition text-black">
      ...
      {/* Modal de Edição */}
      {editModalOpen && selectedCategoria && (
        <div className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-sm z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-[90%] max-w-md text-center relative">
            <button
            ...
            <h2 className="text-xl font-semibold text-[#36B0AC] mb-4">Editar Categoria</h2>
            ...
            <div className="flex flex-col gap-4">
              <input
                ...
                onChange={(e) =>
                  setSelectedCategoria({ ...selectedCategoria, nome: e.target.value })
                }
              ...
              <label className="flex items-center justify-center gap-2 text-black">
                <input
                  type="checkbox"
                  checked={selectedCategoria.ativo}
                  onChange={(e) =>
                    setSelectedCategoria({
                      ...selectedCategoria,
